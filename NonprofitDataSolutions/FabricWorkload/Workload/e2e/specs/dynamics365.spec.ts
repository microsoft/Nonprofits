...
/**
 * Full E2E flow — Dynamics 365 package.
 *
 * Creates a Fundraising item, selects the Dynamics 365 optional package,
 * completes the additional configuration (lakehouse selection), deploys,
 * and verifies success.
 */
test.describe('Full E2E Flow — Dynamics 365', () => {
	test('create item, configure D365 lakehouse, deploy, and verify success', async ({
		workspacePage,
	}) => {
		...
		const itemName = `PW_D365_${Date.now()}`;
		...
		// Verify Dynamics 365 integration section is shown
		await expect(wizard.dynamicsIntegrationHeading).toBeVisible();
		await expect(wizard.lakehouseDropdown).toBeVisible();

		// Select a lakehouse with "dataverse" in the name

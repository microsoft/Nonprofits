...
try
{
    ...
    $AccessKey = (Get-AzureRmStorageAccountKey -ResourceGroupName $ResourceGroupName -AccountName $StorageAccountName).Value[0];
    ...
    foreach ($configuration in $PermissionsConfiguration)
    {
        ...
        foreach($permission in $configuration.Permissions)
        {
            ...
            #Obtain Principal depending on the type
            if(($permission.Type -eq "g") -or ($permission.Type -eq "default:g"))
            {
                $principal = Get-AzureRmADGroup -SearchString $permission.Principal;
            }
		    elseIf(($permission.Type -eq "u") -or ($permission.Type -eq "default:u"))
            {
                $principal = Get-AzureRmADUser -DisplayName $permission.Principal;
            }
            elseIf(($permission.Type -eq "sp" -or ($permission.Type -eq "default:sp")))
            {
                $principal = Get-AzureRmADServicePrincipal -DisplayName $permission.Principal;
            }
            elseIf(($permission.Type -eq "o" -or ($permission.Type -eq "default:o")))
            {
                ## Crete dummy object for "Other" as they do not have id
                $principal=@{
                    id = "";
                };
            }
            ...
            {
                throw "Principal type not found. Allowed values: g, default:g, u, default:u, sp, default:sp, o, default:o.";
            }

            if($null -eq $principal)
            {
                Write-Host "Principal $($permission.Principal) does not exists." -ForegroundColor Yellow;
            }
            else
            {

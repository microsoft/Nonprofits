...
.SYNOPSIS
    Main Deployment Orchestrator for Non-Profit Data Warehouse Quick Start Solution
...
.PARAMETER subscriptionId
   Specifies Azure Portal SubscriptionId (GUID) Where resources will be deployed. 
   This value should be obtained from Azure Portal 
...
.PARAMETER Tags
    ...
    e.g.
    Tags = @{Environment = "Dev"; Project = "NonprofiDataWarehouseQuickStart"},
...
.PARAMETER DeveloperGroupName
    ...
    e.g.
    "AAD-GRP-QUICKSTART-DEV-DEVELOPER",
...
.PARAMETER AdminGroupName
    ...
    Group need to be provided. Group will not be created as a part of this script and need to be precreated.
    e.g.
    "AAD-GRP-QUICKSTART-DEV-ADMIN"

.OUTPUTS
   No objects are outputed

.EXAMPLE
    ./DeployAzureConfiguration -SubscriptionId "" -OrganisationPublicIPAddress "" -Project "mstsidh"
...
param(
    ...
    [Parameter(Mandatory=$true)]
    [ValidateNotNullOrEmpty()]
    [string] $OrganisationPublicIPAddress = "",
    ...
    [Parameter(Mandatory=$false)]
    [string] $ResourceGroupName = "Nonprofit_Data_Warehouse_Quickstart_Demo2",
    ...
    [Parameter(Mandatory=$false)]
    [ValidateNotNullOrEmpty()]
    [string] $Environment="demo4",

    [Parameter(Mandatory=$false)]
    [ValidateNotNullOrEmpty()]
    [object] $Tags = @{Environment = "Demo4"; Project = ""},

    [Parameter(Mandatory=$false)]
    [ValidateNotNullOrEmpty()]
    [string] $DeveloperGroupName = "AAD-GRP-QUICKSTART-DEV-DEVELOPER-ELUCHT",

    [Parameter(Mandatory=$false)]
    [ValidateNotNullOrEmpty()]
    [string] $AdminGroupName = "AAD-GRP-QUICKSTART-DEV-ADMIN-ELUCHT"
)
...
# resources names
$synapseAnalytics = "QuickStart";
$synapseAnalyticsAdminLogin = "quickstartadmin";
...
try
{
    ...
    try
    {
        $existingFirewallRule = Get-AzSqlServerFirewallRule `
            -ResourceGroupName $resourceGroup `
    ...
    $adlsConfiguration = @(
       ...
       @{
            Folder="RAW/IATI/ActivityDate";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/ActivityStatus";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/Budget";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/ContactInformation";
            Permissions=$defaultPermissionConfiguration
        }
       @{
            Folder="RAW/IATI/Description";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/ParticipatingOrganisation";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/RecipientCountry";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/RecipientRegion";
            Permissions=$defaultPermissionConfiguration
        }
       @{
            Folder="RAW/IATI/ReportingOrganisation";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/WHO";
       ...
       @{
            Folder="RAW/WHO/Sanitation";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/Sector";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/Title";
            Permissions=$defaultPermissionConfiguration
        },
       @{
            Folder="RAW/IATI/Transaction";
            Permissions=$defaultPermissionConfiguration
        }
    );
    ...
    $deploySynapseAnalyticsSqlConnectionString = "Server=tcp:$($sqlServer).database.windows.net,1433;Initial Catalog=$($synapseAnalytics);Persist Security Info=False;User ID=$($synapseAnalyticsAdminLogin);Password=$($synapseAnalyticsAdminPassword);MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;";  
    .\SqlServerCode\Execute-DeploySqlServerCode.ps1 `
        -ConnectionString $deploySynapseAnalyticsSqlConnectionString `
        ...
        -CommandParameters @(
            ...
            @{SearchString="%STORAGE_ROOT_URI%"; ReplaceValue="abfss://quickstart@$($dataLakeStore).dfs.core.windows.net"}
        ) `
        -ErrorAction Stop; 
    ###---------------------------  
